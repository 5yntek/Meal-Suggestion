...
package org.xyntek.mealsuggestion;
...
public class PrePostProcessor {
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        results = removeNonFood(results);
        return nonMaxSuppression(results, mNmsLimit, mThreshold);
    }

    private static Boolean validClass(int classID){
        final int BOTTLE = 40;
        final int BANANA = 47;
        final int APPLE = 48;
        final int SANDWICH= 49;
        final int ORANGE = 50;
        final int BROCCOLI = 51;
        final int CARROT = 52;
        final int HOT_DOG = 53;
        final int PIZZA = 54;
        final int DONUT = 55;
        final int CAKE = 56;
        return Arrays.asList(BOTTLE, BANANA, APPLE, SANDWICH, ORANGE,
                BROCCOLI, CARROT, HOT_DOG, PIZZA, DONUT, CAKE).contains(classID);
    }

    private static ArrayList<Result> removeNonFood(ArrayList<Result> results) {
        return results.stream().filter(r -> validClass(r.classIndex)).collect(Collectors.toCollection(ArrayList::new));
    }
}
